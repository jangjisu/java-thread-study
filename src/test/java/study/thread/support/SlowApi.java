...
/**
 * 한 번 호출에 {@link #CALL_MILLIS}ms 가 걸리는 외부 API. 모든 step 에서 공용으로 쓴다.
 *
...
public final class SlowApi {
    ...
    public static String call(String name) {
        Sleeper.millis(CALL_MILLIS);
        return name + "-결과";
    }

    /** 호출 시작과 응답 시각을 타임라인에 찍으면서 호출한다. */
    public static String callWithLog(String name) {
        Timeline.log(name + " 호출 시작");
