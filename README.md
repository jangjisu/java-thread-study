...
**테스트는 내가 직접 쓴다.** 각 단계는 `fail("아직 작성하지 않았다")`인 빈 과제로 시작한다.
과제와 힌트만 주어지고, 답은 주어지지 않는다. 읽어서 아는 것과 써서 아는 것은 다르기 때문이다.

1. 그 단계의 테스트 파일을 열면 과제 목록이 있다
2. 하나씩 채워서 초록불로 만든다
3. 알게 된 것을 `docs/step0N-*.md`의 "배운 것"에 자기 문장으로 적는다
4. 막히면 물어본다 — 답이 아니라 힌트를 받는다
